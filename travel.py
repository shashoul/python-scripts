...
class Flight:
    """A flight with a prticular passenger aircraft."""
    ...
    def relocate_passenger(self, from_seat, to_seat):
        ...
        if self._seating[to_row][to_letter] is not None:
            raise ValueError(f"The new seat is already occupied!!")
        
        from_row , from_letter = self._parse_seat(from_seat)

        if self._seating[from_row][from_letter] is None:
            raise ValueError(f"No existing passenger Found!!")
    ...
    def _parse_seat(self, seat):
        ...
        if row not in rows:
            raise ValueError(f"Invald row number {row}")
    ...
    def make_boarding_card(self, card_print):
        for passenger, seat in sorted(self._passenger_seats()):
            card_print(passenger, seat, self._number, self._aircraft.model())
    ...
    def _passenger_seats(self):
        ...
        for row in row_numbers:
            for letter in seat_letters:
                if self._seating[row][letter] is not None:
                    yield self._seating[row][letter], str(row)+letter 
