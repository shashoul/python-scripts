...
class LevelOrderIterator:
    ...
    def __init__(self, sequence):
        if not _is_perfect_length(sequence):
            raise ValueError(
            f"Sequence of length {len(sequence)} does not represet "
            "a perfect binary tree length 2n - 1"
        )
...
class InOrderIterator:
    ...
    def __next__(self):
        ...
        # Push left childern onto the stack while possible
